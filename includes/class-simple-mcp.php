...
class Simple_MCP {
    ...
    const PERMS = ['mcp', 'blocks', 'wploc', 'content', 'wc', 'mc', 'seo', 'wp_cli', 'server_ops'];
    ...
    static function role_defaults($role) {
        ...
        switch ($role) {
            case 'administrator':
                return ['mcp' => true, 'blocks' => true, 'wploc' => true, 'content' => true, 'wc' => true, 'mc' => true, 'seo' => true, 'wp_cli' => true, 'server_ops' => false];
            case 'editor':
                return ['mcp' => true, 'blocks' => true, 'wploc' => true, 'content' => true, 'wc' => true, 'mc' => true, 'seo' => true] + $off;
            case 'author':
                return ['mcp' => true] + $off;
            default:
                return $off;
        ...
        if (is_array($raw) && (isset($raw['wp_cli_enabled']) || isset($raw['modules']) || isset($raw['user_id']) || isset($raw['allow_server_ops']))) {
            ...
            if (isset($raw['modules']) && is_array($raw['modules'])) {
                foreach ($roles as $slug => $p) {
                    foreach (['blocks', 'wploc', 'content', 'wc', 'mc', 'seo'] as $m) {
