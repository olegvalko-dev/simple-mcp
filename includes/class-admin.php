...
class Simple_MCP_Admin {
    ...
    /** Аддон-групи та наявність супутнього плагіна: perm => found. */
    static function addon_perms() {
        return [
    ...
    static function perm_rows() {
        return [
            ...
            'wc'         => ['WooCommerce-синк', 'wc_sync_product / wc_synced_meta_keys — товарні дані (ціни, сток, SKU, варіації) між мовами. Потребує wp-loc-woocommerce.'],
            'mc'         => ['Мультивалютність', 'mc_get_config / mc_set_rate / mc_set_product_prices — курси й пер-валютні ціни. Потребує wp-loc-multicurrency.'],
            'seo'        => ['SEO (AIOSEO)', 'seo_get / seo_update / seo_get_strings / seo_update_strings — SEO-поля по мовах і глобальні рядки. Потребує wp-loc-aioseo.'],
    ...
    static function handle_save() {
        ...
        foreach (array_keys(wp_roles()->roles) as $slug) {
            ...
            // Аддон-групи без свого плагіна → чекбокс disabled і не постить: зберігаємо попередній намір
            foreach (self::addon_perms() as $perm => $found) {
                if (!$found) $p[$perm] = (bool) Simple_MCP::role_perms_raw($slug)[$perm];
            }
                    ...
                    <?php foreach (self::perm_rows() as $perm => [$label, $hint]): ?>
                        <tr>
                            ...
                            <?php foreach ($wp_roles as $slug => $info):
                                ...
                                $addons = self::addon_perms();
                                if (isset($addons[$perm]) && !$addons[$perm]) {
                                    $disabled = true;
                                    $title    = 'Супутній плагін не активний — група прихована для всіх';
                                }
                                $checked = !$disabled && !empty($raw[$perm]);
                                if ($perm === 'wploc' && !$ml) $checked = false; // приховано — не показуємо як активне
                                if (isset($addons[$perm]) && !$addons[$perm]) $checked = false;
                            ?>
